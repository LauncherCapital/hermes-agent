...
# ---------- Permissions ----------
# The Ringo Railway image runs the API server only. It uses the baked frontend,
# baked Python dependencies, and HERMES_TUI_DIR fast path, so it does not need
# runtime-writable dependency trees. Avoid the recursive chmod/chown layer here:
# on Railway it dominates the build and can stall deployment on node_modules.
USER root
