...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    _r = _ra()
    host_managed = bool(getattr(agent, "_host_managed_prompt", False))
    ...
    if not _soul_loaded and not host_managed:
    ...
    if tool_guidance and not host_managed:
        stable_parts.append(" ".join(tool_guidance))

    # Computer-use (macOS) — goes in as its own block rather than being
    # merged into tool_guidance because the content is multi-paragraph.
    if not host_managed and "computer_use" in agent.valid_tool_names:
    ...
    if agent.valid_tool_names and not host_managed:
    ...
    if agent.provider == "alibaba" and not host_managed:
    ...
    _env_hints = "" if host_managed else _r.build_environment_hints()
    if _env_hints:
    ...
    if not host_managed and getattr(agent, "_environment_probe", True):
        try:
    ...
    if not host_managed and platform_key in PLATFORM_HINTS:
        stable_parts.append(PLATFORM_HINTS[platform_key])
    elif platform_key and not host_managed:
    ...
    if agent._memory_store and not host_managed:
    ...
    if agent._memory_manager and not host_managed:
        try:
